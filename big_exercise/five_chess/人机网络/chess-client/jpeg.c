...
u32_t * rgb24to32(u8_t *buf24, fb_info picture_inf)
{
    ...
    {
        *((u8_t *)&buf[iloop] + 3) = 0x00;
        *((u8_t *)&buf[iloop] + 2) = 0x00;

        *((u8_t *)&buf[iloop] + 1) = buf24[iloop * 3 + 0] & 0xF8;
        *((u8_t *)&buf[iloop] + 1) = *((u8_t *)&buf[iloop] + 1) | ((buf24[iloop * 3 + 1] >> 5) );
        *((u8_t *)&buf[iloop] + 0) = (buf24[iloop * 3 + 1] << 3) & 0xC0;
        *((u8_t *)&buf[iloop] + 0) = *((u8_t *)&buf[iloop] + 0) | (buf24[iloop * 3 + 2] >> 3);
    }
